...
p_eps <- qsn(tau_p,
             ...
             alpha=a_p)
p_tru <- cbind(p_eps*s_p_non(s_tru_quants[1]) + m_p_non(s_tru_quants[1]),
               p_eps*s_p_non(s_tru_quants[2]) + m_p_non(s_tru_quants[2]),
               p_eps*s_p_non(s_tru_quants[3]) + m_p_non(s_tru_quants[3]),
               p_eps*s_p_non(s_tru_quants[4]) + m_p_non(s_tru_quants[4]),
               p_eps*s_p_non(s_tru_quants[5]) + m_p_non(s_tru_quants[5]))
...
for (m in Ms){
  ...
  # if (m == 1){
    ...
    for (k in 1:K){
      ...
      Bs_quants <- bs(s_tru_quants,degree=degree,knots=s_kn,Boundary.knots = c(0,1),intercept = TRUE)
      # Simulate p given s
      p_tilde_sim <- Bs_quants %*% quant.reg(p_tilde,Bs,Ds,lambda,tau_p[k])$b
      # Simulate y given p and s
      p_tilde_rep <- rep(p_tilde,sim_ctys)
      p_kn <- quantile(p_tilde_rep,c(seq(1/Q,(Q-1)/Q,1/Q)))
      Bp <- bs(as.vector(p_tilde_sim),degree=degree,knots=p_kn,Boundary.knots = c(-2,2),intercept = TRUE) # Bspline basis used to get simulated y for a given tau_y
...
saveRDS(p_vals_s1,"p_vals_s1_pnon.rds")
saveRDS(p_vals_s2,"p_vals_s2_pnon.rds")
saveRDS(p_vals_s3,"p_vals_s3_pnon.rds")
saveRDS(p_vals_s4,"p_vals_s4_pnon.rds")
saveRDS(p_vals_s5,"p_vals_s5_pnon.rds")
saveRDS(p_sim,"p_sim_pnon.rds")
saveRDS(y_sim,"y_sim_pnon.rds")
saveRDS(p_tru,"p_tru_pnon.rds")
saveRDS(y_tru,"y_tru_pnon.rds")
